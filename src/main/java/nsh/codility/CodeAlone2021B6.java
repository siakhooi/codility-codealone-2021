...
public class CodeAlone2021B6 implements CodeAlone2021Interface {
	// B6
	...
	public int solution(String S) {
		...
		if (max[0] < 3 && max[1] < 3) {
			...
			if (has2211)
				return 2;
			if ((max[0] == 1 && min[1] == 2) || (max[1] == 1 && min[0] == 2))
				return 4;
		...
		for (int j = 0; j < index2Count && m > 2; j++) {
			int i = index2[j];
			if (i > 1) {
